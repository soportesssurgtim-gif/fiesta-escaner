    ...
    const app = createApp({
      setup() {
        ...
        function abrirGuiaVista() {
          ...
          const data = (window.GUIAS_VISTAS && window.GUIAS_VISTAS[clave]) ? window.GUIAS_VISTAS[clave] : (window.GUIAS_VISTAS && window.GUIAS_VISTAS.scanner) || { titulo: 'GuÌa', icono: 'question-circle', color: '#4b5563', pasos: [], tips: '' };
        ...
        async function consultarInvitacionPublica() {
          ...
          if (!duiPublico.value || !ultimos4Publico.value) {
            errorPublico.value = 'Ingresa tu DUI y los ˙ltimos 4 dÌgitos.';
            return;
          }
          ...
          try {
            ...
            if (!res.ok) {
              errorPublico.value = data.error || 'No se pudo consultar la invitaciÛn.';
              return;
            }
            resultadoPublico.value = data;
          } catch (err) {
            errorPublico.value = err.message || 'Error de conexiÛn.';
          } finally {
